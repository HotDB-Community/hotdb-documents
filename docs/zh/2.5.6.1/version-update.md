...
为了更好的帮助用户对管理平台以及计算节点版本进行升级操作，特此编辑该文档。该文档旨在帮助用户通过文档操作说明，独立手动完成管理平台与计算节点的基本升级。同时提供对版本升级出现异常时的处理意见。
...
计算节点版本升级支持手动和自动两种方式，自动升级请参考[管理平台](hotdb-management.md)文档的升级中心菜单说明。以下将通过手动的方式对单节点、主备节点、多节点模式的集群进行计算节点版本升级说明。
...
若计算节点版本由低版本升级到V2.5.6（包含）以上版本时，计算节点许可证升级为官方自研许可证，原程序包内`keys`目录下许可证将无法使用，直接废弃；新版本包内`keys`目录下默认存放90天、16节点试用版许可证，启动计算节点时自动激活。**若发现当前已有的节点数大于默认试用版的节点数**，则需要在升级之前参考[许可授权](service-license.md)文档（2.5.6版本以上）进行机器码获取及许可证更新操作，否则可能导致计算节点服务无法正常启用。
...
多节点集群模式选择不停机升级要求必须满足以下前提才能进行，否则只能在[停机条件下对计算节点版本进行升级](#容灾模式的集群升级)操作。
...
对一个开启容灾模式的计算节点集群做手动升级，需要以机房为单位进行升级。

若机房发生切换，建议优先修复机房故障后再进行升级操作，即容灾模式的集群建议在两个机房内无组件异常且机房间复制状态正常时进行手动升级。

容灾模式的集群升级必须先升级中心机房，确保升级无误后，再升级容灾机房。

#### 中心机房升级

容灾模式的中心机房升级与单机房模式的集群升级大致相同。
...
因容灾机房不提供真实的数据库服务，故在日常运维过程中，容灾机房的计算节点仅开启管理端口或处于停机状态。

若容灾机房的计算节点为单节点模式，请参考[单节点集群模式停机升级](#单节点集群模式升级)。

若容灾机房的计算节点为主备节点模式，请参考[主备节点集群模式停机升级](#主备节点集群模式停机升级)或[不停机升级](#主备节点集群模式不停机升级)。
