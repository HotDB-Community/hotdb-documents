...
- 更多->机房切换演练：可参考[可视化机房文档](visual-jdc.md)
...
**说明：**通过打分的方式体现集群中水平分片表的拆分优劣情况，当计算节点启动不足24小时得分无法计算，页面不予显示。点击标题处会跳转到"[数据分片评分](#数据分片评分)"详情界面。评分趋势图显示近期分片合理性评分变化情况，可以选择按天查看和按小时查看。
...
- `show processlis`查询后，鼠标移入表头会显示具体字段的解释信息
...
灾备模式说明：集群开启灾备模式时，一键迁库相关逻辑说明请结合[跨机房灾备](cross-idc-disaster-recovery.md)文档中的[一键迁库](cross-idc-disaster-recovery.md#一键迁库)章节。
...
- `insert/replace`的values子句2列表、`insert set/insert onduplicate set`的set子句列表、`update set`子句列表中，不判断常量数值是否小于10或"Y"、"N"等特殊含义常量**（在V2.5.3及以上版本中生效）**
