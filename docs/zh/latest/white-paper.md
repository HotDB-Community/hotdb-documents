...
> !!!NOTE
> 以上所述TPS为简单TPS。以上所述性能指标均基于：使用《分布式事务数据库产品HotDB Server硬件配置推荐---热璞科技》所述最高配置硬件规格，规范地安装部署并进行《HotDB Server单计算节点_基础场景性能测试报告》所述调优，不少于12个数据节点，存储节点服务器/MySQL实例未出现性能瓶颈的前提。
