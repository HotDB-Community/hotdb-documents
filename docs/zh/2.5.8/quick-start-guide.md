...
9. 堆内存默认为4G，当设置大于8G时，会提示要求开启“G1垃圾回收器”。该参数输入范围【1-64】G。但是需要注意：1、管理平台版本大于等于2.5.7时，"G1垃圾回收器"默认开启；2、部署计算节点使用JDK11时，垃圾回收器默认为"ZGC垃圾回收器"；3、ARM架构的CPU不支持"ZGC垃圾回收器"，当前建议使用JDK8和"G1垃圾回收器"。
