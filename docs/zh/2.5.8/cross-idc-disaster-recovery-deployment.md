...
此手册基于分布式事务数据库产品 HotDB Server - V2.5.8版本编写，主要讲述基于MySQL原生复制功能实现HotDB Server跨机房灾备方案所涉及的功能基本使用方法及操作流程，供使用者参考与学习。

此文档中涉及较多计算节点、管理平台相关基础功能，若需了解标准计算节点、管理平台的使用方法，请参考请参考[管理平台](hotdb-management-platform.md)文档和[计算节点标准操作](hotdb-server-standard-operations.md)文档。
...
![6images-1](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-1.png)
...
**依赖软件：** 运行HotDB Server（分布式事务数据库）与HotDB Management（分布式事务数据库管理平台）需要依赖JAVA环境，计算节点版本低于2.5.6要求使用JDK1.7版本，高于2.5.6 需要使用JDK_1.8或JDK_11版本。

**网络环境：**单 个机房内至少千兆网卡、跨机房服务器间网络总延迟小于 10ms。跨机房间带宽完全充足，不存在任何带宽瓶颈，无任何丢包情况。
...
![6images-2](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-2.png)

各组件对应的实际连接信息，列表列举如下，部分截图内容需与之对应：

| 机房类型 | 组件类型 | 角色   | 名称代号    | 连接信息                                                  |
| -------- | -------- | ------ | ----------- | --------------------------------------------------------- |
| 中心机房 | 计算节点 | 主     | 计算节点-01 | 192.168.210.135_3323_3325                                 |
| ^        | ^        | 备     | 计算节点-02 | 192.168.210.136_3323_3325                                 |
| ^        | 存储节点 | 主     | ds01        | 192.168.210.135_3200                                      |
| ^        | ^        | 双主备 | ds02        | 192.168.210.136_3200                                      |
| ^        | 配置库   | 主     | hc01        | 192.168.210.135_3306                                      |
| ^        | ^        | 双主备 | hc02        | 192.168.210.136_3306                                      |
| 灾备机房 | 计算节点 | 主     | 计算节点-01 | 192.168.210.78_3323_3325                                  |
| ^        | ^        | 备     | 计算节点-02 | 192.168.210.79_3323_3325                                  |
| ^        | 存储节点 | 主     | ds03        | 192.168.210.78_3200                                       |
| ^        | ^        | 双主备 | ds04        | 192.168.210.79_3200（配置为双主，实际复制关系搭建为主从） |
| ^        | 配置库   | 主     | hc03        | 192.168.210.78_3306                                       |
| ^        | ^        | 双主备 | hc04        | 192.168.210.79_3306（配置为双主，实际复制关系搭建为主从） |
...
![6images-3](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-3.png)
...
![6images-4](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-4.png)

因此，中心机房参数配置参考如下（[部署架构](#部署架构)）：

![6images-5](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-5.png)

![6images-6](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-6.png)
...
![6images-7](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-7.png)

管理平台的安装部署进程中增加步骤【搭建灾备关系】，但在部署中心机房时，此步骤中不做额外操作。故部署中心机房的安装流程与单机房模式下的部署一个集群相同。

![6images-8](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-8.png)

**（四）集群管理：**

仅部署完成中心机房后，若没有部署或添加灾备机房，开启对此集群的监控后，此中心机房可视为一个单机房模式下的计算节点集群，皆按照单机房模式进行管理。

![6images-9](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-9.png)

![6images-10](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-10.png)

即，中心机房部署完成后，在集群管理页面进去该机房计算节点详情页面，可以看到此时，该集群的灾备模式为关闭状态。

![6images-11](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-11.png)
...
![6images-12](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-12.png)
...
![6images-13](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-13.png)

- 时间同步地址和操作系统时区必须与中心机房保持一致。

因此，灾备机房参数配置参考如下（此处可回顾前文提到的[部署架构](#部署架构)，再做参数填写）：

![6images-14](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-14.png)

![6images-15](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-15.png)

**（三）开始部署：**

1\. 点击【检查并保存】检查无误后，点击【开始部署】进入安装进程。

2\. 在步骤【生成基础配置】中，灾备机房的配置将同步更新到中心机房的配置库和`server.xml`，并完成动态加载后生效。
...
![6images-16](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-16.png)

##### 集群管理

灾备机房安装部署成功后，在集群管理页面就可以看到一个开启灾备模式且有两套机房的计算节点集群。

![6images-17](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-17.png)

- 机房类型分为中心机房和灾备机房。根据机房状态，在机房类型旁用![](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-129.png)图标表示该机房类型为当前主机房；

- 两个机房拥有各自的组件配置信息和部署信息，其中当前备机房不提供【切换】和【重建】的入口，即当前备机房不允许手动切换计算节点；

- 一个集群中的两个机房暂不提供分开停止监控或开启监控，用户权限也将两个机房视作一个集群，统一管理；

![6images-18](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-18.png)
...
![6images-19](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-19.png)
...
![6images-20](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-20.png)
...
| ^        | ^        | 双主备（实际为主从复制关系） | hc04     | 192.168.220.189_3306 |
...
此时在220.186和220.187上分别执行show slave status可以看到 Master_Host的关系互为双主，且`Slave_IO_Running`和`Slave_SQL_Running`状态正常，例如：
...
从零到一地部署一套新灾备环境下，配置库和存储节点实质上都是一个标准的MySQL新实例，区别在于配置库的复制关系搭建完成后需要将配置库数据导入而存储节点复制关系搭建完成即可。因此存储节点的灾备关系搭建过程请参照[配置库容灾关系搭建](#多计算节点集群模式)，并跳过配置库数据导入步骤。
...
![6images-21](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-21.png)
...
>
>    ![6images-22](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-22.png)
>
>    当添加的计算节点集群模式与真实的计算节点模式不匹配时，配置校验会同步校验出当前状态并给予错误提醒
>
>    ![6images-23](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-23.png)

**（二）配置库：**

若计算节点管理端口可连接，则管理平台会自动获取配置库信息，无需手动配置配置库；若计算节点管理端口不可连接，则管理平台无法获取配置库信息，需要手动添加配置库信息。勾选【手动设置配置库】后，分别填写两个机房的配置库信息。

![6images-24](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-24.png)

**（三）测试连接：**

点击【测试】测试计算节点连接是否正常。

![6images-25](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-25.png)*连接正常示例图*

![6images-26](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-26.png)*连接异常示例图*
...
![6images-27](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-27.png)
...
在已运行的集群基础上部署灾备机房，需要对中心机房的灾备模式相关[新增参数](#新增参数)做修改，修改内容可参考部署一套新容灾环境的[计算节点参数配置修改](#计算节点参数配置修改)。
...
![6images-28](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-28.png)

1\. 在集群管理页面中，点击一个已运行的集群名称，进入到计算节点集群编辑页面。

2\. 点击【开启灾备模式】，则将已有集群信息作为中心机房配置信息，并为其添加灾备机房的配置信息，详情请参考[集群添加](#计算节点集群添加)。

![6images-29](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-29.png)
...
此章节需要结合[管理平台](hotdb-management-platform.md)文档一起阅读。此章节仅描述管理平台管理的计算节点集群组开启灾备模式之后的功能使用说明，区别于普通未开启灾备模式的情况。同时，未特殊提及的功能页面均默认显示为当前主机房相关信息、控制当前主机房的状态。
...
![6images-30](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-30.png)
...
![6images-31](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-31.png)*上图为中心机房故障，灾备机房切换为当前主机房*

- 若中心机房所有计算节点都无法连接，则点击任意机房面板，显示当前中心机房无法提供服务的提示信息。请确保主机房的所有计算节点被关闭后，人工启动灾备机房。若需要启动灾备机房，请参考[中心机房故障后切换至容灾机房](#中心机房故障后切换至容灾机房)。

![6images-32](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-32.png)
...
![6images-33](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-33.png)
...
![6images-34](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-34.png)
...
![6images-35](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-35.png)

**（一）增加的列表信息说明：**

- **机房类型：** 指该数据节点所属机房类型，中心机房或灾备机房。根据机房状态显示颜色：可用 - 绿色、备用 - 蓝色、不可用 - 红色。机房状态与[计算节点集群选择](#计算节点集群选择)页面显示的机房状态一致。
...
![6images-36](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-36.png)
...
![6images-37](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-37.png)
...
![6images-38](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-38.png)

点击【确定】后，管理平台将自动生成灾备机房缺失节点，节点类型与中心机房一致。可以修改【数据节点类型】或【存储节点组】后重新生成，但数据节点名称与个数不能修改，仍然与中心机房保持一致。

![6images-39](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-39.png)

**（四）导入功能：**

使用导入功能时，需要保证导入的中心机房与灾备机房的数据节点逻辑架构一致，即数据节点个数相同，名称一一对应，否则将导入失败。

![6images-40](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-40.png)

##### 主从搭建

主从搭建不仅可以为单个机房内的尚未搭建复制关系的存储节点搭建复制关系，还可以为两个机房主存储节点之间搭建灾备关系。

![6images-41](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-41.png)

**（一）搭建须知：**

（仅说明灾备机房的不同，详细的搭建须知与步骤请参考[管理平台](hotdb-management-platform.md)文档）
...
![6images-42](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-42.png)
...
![6images-43](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-43.png)
...
![6images-44](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-44.png)
...
![6images-45](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-45.png)
...
![6images-46](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-46.png)

#### 计算节点参数配置

计算节点参数配置可以可视化的配置两个机房的计算节点server.xml参数

![6images-47](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-47.png)
...
![6images-48](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-48.png)
...
![6images-49](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-49.png)
...
![6images-50](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-50.png)

2、若确定主配置库已恢复正常且与备配置库数据一致且最新时，可点击[计算节点参数配置](#计算节点参数配置)页面启用按钮对主配置库进行启用。

3、点击![6images-51](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-51.png)启用按钮后，再点击【动态加载】按钮，即可将主配置库重新启用

![6images-52](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-52.png)
...
>
>   ![6images-53](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-53.png)

**（四）新增参数：**

1\. 配置灾备机房的配置库

![6images-54](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-54.png)
...
- 灾备模式下，需要配置参数`idcId`和`idcNodeHost`&nbsp;

- `idcId`(机房ID)：用户需要设置一个ID值来唯一的标识机房类型，目前默认设置为1表示中心机房，设置为2表示灾备机房

- `idcNodeHost`(另一个机房计算节点信息)：填写另一个机房的所有计算节点连接信息，配置格式为：IP:PORT

  例如中心机房主计算节点连接信息为`192.168.220.213:3325`，备计算节点连接信息为`192.168.220.214:3325`；灾备机房主计算节点连接信息为`192.168.220.217:3325`，备计算节点连接信息为`192.168.220.218:3325`，则在灾备机房的`server.xml`中，参数`idcNodeHost`应配置示例如下：
...
![6images-55](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-55.png)

- 2.5D主拓扑同样增加配置库组件，功能与主拓扑一致。

![6images-56](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-56.png)

- ![6images-57](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-57.png)为无状态图标，该图标连接一个或两个配置库![6images-58](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-58.png)，分别表示单节点配置库和双主配置库。在单机房模式下，若连接三个及以上的配置库，则代表MGR模式的配置库。当所有配置库不可用时，![6images-59](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-59.png)会显示为橙色。
...
![6images-60](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-60.png)
...
![6images-61](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-61.png)

**（二）灾备拓扑：**

- 在主拓扑或2.5D主拓扑页面，点击【![6images-62](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-62.png)】切换至灾备拓扑，若再次点击则回切至主拓扑或2.5D主拓扑页面。

- ![6images-63](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-63.png)的角标代表，在灾备拓扑中，灾备机房中的当前为error状态且未修复的组件个数，也就是历史信息面板中当前存在仍未被修复的error信息数，包括实际无法连接的组件数以及两个机房之间的存储节点或配置库复制状态异常。

  若中心机房故障，灾备机房切换为当前主机房，则主拓扑此时显示为灾备机房拓扑图，灾备拓扑的角标仍会显示灾备机房当前error数。

![6images-64](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-64.png)

- 灾备拓扑中从左到右共有六层组件，从左到右1-3层依次表示中心机房的计算节点、数据节点（配置库）和存储节点（配置库）；4-6层依次表示灾备机房的存储节点（配置库）、数据节点（配置库）和计算节点

![6images-65](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-65.png)

- 一个机房内部的连线代表含义与主拓扑一致。若灾备机房主配置库或主存储节点服务异常，则灾备机房的对应数据节点下的所有存储节点也置为红色不可用状态且连线为灰色。

![6images-66](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-66.png)*图为灾备机房配置库服务异常*

- 两个机房之间的连线，即中心机房当前主存储节点和灾备机房当前主存储节点之间的连线表示灾备状态。若灾备机房状态异常，则机房之间连线为灰色

![6images-67](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-67.png)*图为灾备机房配置库灾备关系复制异常*

- 中心机房和灾备机房中存在复制时延的存储节点会显示复制时延的监控信息，若复制时延为0，则不显示。复制时延与主拓扑一样，都在“设置 —\> 拓扑图报警设置”中的配置库或存储节点模块下设置报警阈值。
- 灾备机房的当前主计算节点，会显示灾备时延的监控信息。配置库和存储节点的灾备时延统一在灾备拓扑页面的【![6images-68](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-68.png)】中设置报警阈值。

![6images-69](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-69.png)
...
![6images-70](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-70.png)

- 若为多计算节点集群模式下的灾备架构，则界面显示示例图如下，其功能同主备模式类同：

![6images-71](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-71.png)

**（三）灾备拓扑的历史信息面板：**

![6images-72](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-72.png)
...
![6images-73](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-73.png)
...
![6images-74](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-74.png)
...
![6images-75](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-75.png)

**（三）网络质量**

可查看跨机房网络质量监控信息，跨机房网络质量拓扑与单机房网络质量拓扑有部分区别。

![6images-76](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-76.png)
...
![6images-77](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-77.png)

如果发生机房切换，即当前灾备机房的计算节点提供服务，则仅展示灾备机房的网络质量监控状态， 中心机房所有组件均置灰显示不做任何监控。且不存在灾备机房到中心机房的网络连线，同时灾备机房的网络连线关系退化成单机房一致。

**（四）监控信息管理：**

监控信息管理是可以通过计算节点管理端口，查看两个机房的计算节点前后端连接之间的关系及其他有效管理信息等

![6images-78](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-78.png)

若【查询命令】选择【服务端连接信息：show processlist】，则默认显示当前主机房的主计算节点服务端连接信息：

![6images-79](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-79.png)
...
![6images-80](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-80.png)

#### 一键迁库

管理平台支持将中心机房或灾备机房的存储节点迁移到新的存储节点，并重新搭建灾备关系。

![6images-81](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-81.png)

- 可以通过机房类型，选择迁移中心机房或灾备机房的存储节点，仍可手动搭建复制关系或通过管理平台搭建复制关系。需要注意的是：配置的目标新数据库实例不能与灾备机房现有实例重合，即检测填写的实例不能与配置库中现有的所有存储节点实例重合。

![6images-82](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-82.png)

- 在【第四步：迁库】中，会对中心机房的当前主存储节点与灾备机房的当前主存储节点搭建复制关系。例如，若中心机房的数据节点进行迁移，管理平台会为中心机房的目标主存储节点和灾备机房的当前主存储节点搭建复制关系。
...
![6images-83](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-83.png)

**（一）选择机房类型：**

机房类型为多选下拉框，即可以选择中心机房或灾备机房或同时选择两个机房。

![6images-84](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-84.png)
...
![6images-85](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-85.png)
...
![6images-86](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-86.png)

![6images-87](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-87.png)

**（三）增加检测类型：**

![6images-88](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-88.png)

- 当检测类型选择“仅检测表结构”时，只检测表结构之间是否一致，详情中会提示哪个存储节点表结构不一致。

- 当检测类型选择“同时检测表结构及数据”时，检测表结构及表数据是否一致，详情中会提示哪个存储节点表结构或数据不一致。

### 事件

#### 通知策略

添加通知策略可以通过邮件和短信的方式对集群运行中中心机房或灾备机房发生的故障或异常问题进行告警提示。监控范围中【故障实时监控】、【计算节点服务器资源监控】、【计算节点服务状态】、【存储节点信息监控】、【配置库信息监控】、【集群资源监控】和【许可证授权监控】的邮件和短信内容增加机房标识，提醒机房相关组件的故障、异常或超出预警值的信息。

**（一）邮件所属信息和短信机房类型：**

邮件所属信息增加：是否开启灾备模式、当前主机房类型以及两个机房的计算节点信息

![6images-89](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-89.png)

- 短信的机房类型区分中心机房和灾备机房

![6images-90](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-90.png)![6images-91](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-91.png)

**（二）存储节点信息监控：**

![6images-92](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-92.png)
...
![6images-93](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-93.png)

**（三）配置库信息监控：**

![6images-94](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-94.png)
...
![6images-95](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-95.png)
...
![6images-96](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-96.png)

**（二）计算节点日志：**

计算节点日志记录中心机房和灾备机房的计算节点在运行过程中产生的日志信息。默认显示当前主机房的主计算节点日志信息，可以多选或查看其他计算节点日志信息。

![6images-97](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-97.png)
...
![6images-98](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-98.png)

#### 存储节点密码

存储节点密码同样增加机房类型的适配，显示所有存储节点对应的机房类型。

![6images-99](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-99.png)

#### 信息收集

管理平台的信息收集工具支持收集两个机房所有组件的用于异常分析的日志与配置文件等。

![6images-100](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-100.png)
...
![6images-101](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-101.png)

升级完成后可查看详细更新日志：

![6images-102](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-102.png)
...
![6images-103](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-103.png)
...
![6images-104](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-104.png)
...
![6images-105](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-105.png)
...
主存储节点发生故障时，双主备存储节点复制追上后（是否等待复制追上受参数waitForSlaveInFailover控制，默认开启等待）会自动开始服务，灾备机房主存储节点会自动重置与原中心机房主存储节点的复制关系，并搭建到中心机房的双主备存储节点的复制关系（灾备机房主存储节点为从机）。

中心机房发生故障的存储节点复制关系为主从非双主，若想要提升原故障存储节点为当前主存储节点，则操作方式配置库主备情况一致，需要优先搭建中心机房当前复制回路，并保证复制无异常，动态加载后可回到原主存储节点。

### 仅灾备机房内部故障

本小节主要描述正常提供服务的集群环境下，如果灾备机房内的核心服务组件出现高可用切换（包括手动或故障）时的功能使用说明。

#### 计算节点

因灾备机房不提供真实的数据库服务，故建议日常运维过程中，直接将灾备机房的主备高可用组件keepalived停止，以规避灾备机房发生VIP漂移后带来不必要的恢复操作。若灾备机房keepalived高可用组件不提供服务，则当灾备机房计算节点发生故障时，不论主备，仅需要在可操作环境下重启服务即可。

若因特殊要求，需要保留灾备机房keepalived的服务，当灾备机房主计算节点故障时，keepalived检测服务可用性超时后，VIP会漂移至备计算节点所在服务器，此时若要保证主计算节点能够正常备用，则需要优先保证VIP能够回到原灾备机房主，即：将备keepavlived的服务停止，启动主计算节点，再操作主keepalived执行reload，最后启动备keepalived服务。

#### 配置库

##### 故障切换

灾备机房主配置库发生故障后，会自动将灾备机房的双主备配置库级联置为不可用。当故障恢复后，需要在管理平台“[参数配置](#计算节点参数配置)”页面点击【启用】按钮并动态加载生效。灾备机房的双主备配置库启用时，必须保证其主配置库是启用状态，否则启用无效。若仅灾备机房备配置库发生故障，则仅该备配置库会被标记为不可用，不对其他服务造成影响。

#### 存储节点

##### 故障切换

同配置库逻辑一致，灾备机房的主存储节点发生故障后，该机房内与该故障主存储节点相关的其他从存储节点也会级联被标记为不可用。即ds03发生故障被标记为不可用时，ds04也会联级被标记为不可用。服务恢复后，启用中心机房故障存储节点时，需要优先保证主存储节点是被启用的状态，才能启用备存储节点。

灾备机房主存储节点正常提供服务，但灾备机房备存储节点发生故障后，仅备存储节点及其从机会被标记为不可用，当前正在使用的存储节点不受影响。即ds04发生故障被标记为不可用时，ds03不受影响；但若ds04有其他从机ds05，那么ds05也会被标记为不可用。被标记为不可用的存储节点启用时，要保证原主从复制同步状态无异常。

故障存储节点被启用后，均动态加载之后生效。灾备机房存储节点不支持手动切换。

所有存储节点、配置库手动切换、故障切换后，计算节点会自动调整`rpl_semi_sync_master_wait_for_slave_count`值为正确的值。

### 跨机房故障

本小节主要描述正常提供服务的集群环境下，如果中心机房出现机房级别故障时，切换到灾备机房提供服务过程中需要注意的事项和具体功能使用说明且以主备模式的计算节点集群作为范例。本次说明主要为手动切换机房和修复机房的步骤，若用户需要通过管理平台自动进行机房切换、修复、移除机房动作，则在保证管理平台版本高于（包含）2.5.6基础上，参考[可视化机房切换、修复、移除、演练](visualization-of-idc-functions.md)文档的描述进行操作。

#### 中心机房故障后切换至灾备机房

当中心机房的服务出现无法修复的故障时，可以由人工判断是否切换至灾备机房。当确定切换至灾备机房时，需参考如下章节的描述进行人工预判和检查，以保证切换可以成功。

##### 切换预检测

- 检查所有中心机房主备计算节点均已停止服务，进程均已退出。

- 检查灾备机房各数据节点主存储节点、主配置库服务状态无异常、机房内复制状态无异常（灾备机房计算节点服务启动时也会自动做检查，只是提前人工检查可以提高切换成功的概率）。

- 若之前有启动灾备机房keepalived服务，需要检查灾备机房keepalived的VIP是否在当前主计算节点上（若不涉及计算节点高可用，可跳过该项检查）。
...
![6images-106](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-106.png)

第3步、依旧通过“[集群管理](#集群管理)”-\>“计算节点集群”页面的【集群添加】操作，单独添加当前灾备切换后的新计算节点集群，并配置灾备切换后的配置库信息（默认只要计算节点管理端口当前可连接，此处无需手动再做配置）。

![6images-107](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-107.png)
...
![6images-108](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-108.png)

![6images-109](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-109.png)

![6images-110](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-110.png)

第7步、进入[配置](#配置)->[节点管理](#节点管理)页面删除原中心机房存留数据（可选择按IP搜索，也可选择按不可用状态搜索，但需要注意不要误删当前主计算节点集群所用数据）

![6images-111](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-111.png)

![6images-112](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-112.png)
...
- 需要手动对原中心机房存储节点、配置库的复制关系进行清理动作，即reset slave all。同时，在原中心机房数据可放弃或[[必须放弃](#什么情况下必须放弃原机房数据)](\l)的情况下，清空数据，并执行reset master操作；
...
- 若开启半同步复制，则根据需要手动修改当前主机房主存储节点、主配置库的`rpl_semi_sync_master_wait_for_slave_count`值到正确的个数，与其真实从机个数匹配，参考设置语句：
...
![6images-113](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-113.png)

![6images-114](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-114.png)

第8步、普通账户登录管理平台，选择新添加的灾备集群。

第9步、至[配置](#配置)-> [节点管理](#节点管理)页面添加灾备机房数据节点、存储节点。

![6images-115](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-115.png)

![6images-116](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-116.png)
...
当发生机房级别的故障切换，原灾备机房提升为当前的中心机房后，若原中心机房各个组件服务状态可以恢复，且需要切回原中心机房，所有机房实际角色不变，此时需要人工进行的修改如下：
...
参考[场景一](#场景一废弃原中心机房)、[场景二](#场景二原中心机房作为当前容灾机房)的描述步骤，需要先将当前主机房提升为中心机房，原中心机房作为灾备机房；再做一次机房切换，且重建灾备关系后服务才能回到原中心机房。 即，A、C分别为中心机房主计算节点、灾备机房主计算节点。当第一次发生机房级别故障切换后，C提升为当前主机房计算节点， 若想服务回到A且不停机，则需要先将A状态换成灾备机房，C为中心机房；再在A上执行online_dr命令，使A提升为当前主机房，再将A状态换成中心机房，C换成灾备机房。相当于把场景一、场景二描述的步骤执行两次后，再按中心机房A、灾备机房C的标准进行一次重建。
...
- 需要手动对原中心机房存储节点、配置库的复制关系进行清理动作，即reset slave all ，同时，在原中心机房数据可放弃或[必须放弃](#什么情况下必须放弃原机房数据)的情况下，执行reset master操作；（注意：上一步骤中如果不存在GTID比原灾备机房多的情况，且缺少的GTID不在原灾备机房主存储节点的gtid_purged范围中。也可不执行reset master，尝试直接搭建复制。但是添加完成、数据复制追上以后，一定要进行主从数据一致性校验，以确保数据确实主从一致。）
...
![6images-117](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-117.png)

第8步、登录管理平台，将原中心机房（现灾备机房）因机房自动置为不可用存储节点进行启用：

![6images-118](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-118.png)
...
![6images-119](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-119.png)

![6images-120](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-120.png)

![6images-121](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-121.png)

##### 特殊说明

- 当发生灾备机房切换时，若目标机房存在从库、双主备库类型的存储节点，hotdb.log会输出当前即将要搭建复制关系的主存储节点与从存储节点复制位置，通过复制位置，可以明确知道当前发生机房切换后的存储节点GTID执行情况：
...
当开启灾备模式时，监控整个跨机房集群的各项服务状态是必不可少的工作。在此过程中，必须注意将一些必要的定时任务、监控功能开启，以提前规避风险。

#### 定时任务

建议开启配置库复制状态监测、存储节点复制状态[监控](#监控)；

建议开启定时[主备一致性检测](#主备一致性检测)任务，并检测中心机房、灾备机房的数据一致性。

#### 邮件和短信提醒

建议将所有计算节点、存储节点的服务状态纳入邮件或短信提醒范围，以便及时知晓各核心组件的故障信息。且此次新增配置库复制状态监控，可以在[事件](#事件)-> [通知策略](#通知策略) -> “添加通知策略”页面进行邮件通知和短信通知设置。例如下图配置库复制状态检测示例所示：

![6images-122](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-122.png)

![6images-123](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-123.png)

邮件和短信提醒内容中，会标记具体出现问题的机房类型：

![6images-124](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-124.png)

#### 健康体检

集群正常部署完成后，均建议通过管理平台管理员用户进入[集群管理](#集群管理)->[部署环境体检](#部署环境体检)页面，对中心机房、灾备机房进行环境体检，并按提示，对不合要求的配置进行优化。

![6images-125](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-125.png)
...
![6images-126](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-126.png)

#### 主备一致性校验

任何时候出现主备数据不一致检测记录时，不论中心机房还是灾备机房都应该尽快解决，保证数据一致性；且定时对中心机房、灾备机房数据一致性做校验，若发生机房级别服务切换，则最后一次主备数据一致性检测的时间点前的历史数据可以保证无误。

#### 服务访问

不允许在中心机房运行过程中，通过online_dr命令开启灾备机房服务端口，除非确认要做机房切换；

灾备机房不得与中心机房使用相同的VIP，以规避应用直接跨机房访问的风险；

建议日常运维过程中，直接将灾备机房的主备高可用组件keepalived停止，以规避灾备机房发生VIP漂移后带来不必要的恢复操作。
...
开启灾备模式后，在实际使用和运维过程中必须注意一些异常日志提醒以及在主从复制关系搭建过程中必须注意按正确的方式操作。此章节主要描述该类参考信息。
...
![6images-127](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-127.png)

![6images-128](../../assets/img/zh/6-cross-idc-disaster-recovery/6images-128.png)
...
> **Note**
>
> 采取如上方式导出导入数据，默认会设置了SET @@SESSION.SQL_LOG_BIN= 0;，因此中心机房主实例与灾备机房的复制关系， 和灾备机房本身的复制关系搭建一定要保证千万不能采取先搭好灾备机房间的复制，再导数据的方式，必须将主实例数据先导出，再导入目标新实例之后搭建复制关系。
